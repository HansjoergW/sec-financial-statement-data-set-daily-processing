...
class FillMassParseContent:
    ...
    def prepare_func(data: Tuple[str, str]) -> Tuple[pd.DataFrame, List[ErrorEntry]]:
        ...
        try:
            ...
            return (None, [ErrorEntry(adsh, pre_xml_file, str(e))])
    ...
    def process(self):
        ...
        for i in range(0, len(pre_xml_files_info), 500):
            ...
            result: List[pd.DataFrame, List[ErrorEntry]] = pool.map(FillMassParseContent.prepare_func, chunk)
...
def read_mass_pre_zip_content(
    dbmgr: DBManager, dataUtils: DataAccessTool, year: int, qrtr: int, adshs: List[str] = None
) -> pd.DataFrame:
