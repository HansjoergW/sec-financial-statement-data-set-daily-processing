...
def load_from_raw_xml(base_path: Path, file_prefix: str) -> Tuple[pd.DataFrame, pd.DataFrame, List[ErrorEntry]]:
...
def load_from_raw_xml_2(
    num_file: str, pre_file: str, lab_file: str, adsh
...
def test_merge(
    pre_parsed_df: pd.DataFrame, num_parsed_df: pd.DataFrame, pre_orig_df: pd.DataFrame, num_orig_df: pd.DataFrame
):
    ...
    pre_df = pre_parsed_df[
        pre_parsed_df[common_columns].apply(tuple, axis=1).isin(merged_df[common_columns].apply(tuple, axis=1))
    ]
    num_df = num_parsed_df[
        num_parsed_df[common_columns].apply(tuple, axis=1).isin(merged_df[common_columns].apply(tuple, axis=1))
    ]
