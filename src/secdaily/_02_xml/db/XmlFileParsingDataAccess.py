...
class XmlFileParsingDA(DB):
    ...
    def find_unparsed_numFiles(self) -> List[UnparsedFile]:
        sql = f"""SELECT accessionNumber, xmlNumFile as file, filingDay, filingMonth, filingYear FROM {DB.SEC_REPORT_PROCESSING_TBL_NAME} WHERE xmlNumFile is not NULL and csvNumFile is NULL and numParseState is NULL"""
        return self._execute_fetchall_typed(sql, UnparsedFile)

    def find_unparsed_preFiles(self) -> List[UnparsedFile]:
        sql = f"""SELECT accessionNumber, xmlPreFile as file, filingDay, filingMonth, filingYear FROM {DB.SEC_REPORT_PROCESSING_TBL_NAME} WHERE xmlPreFile is not NULL and csvPreFile is NULL and preParseState is NULL"""
        return self._execute_fetchall_typed(sql, UnparsedFile)

    def find_unparsed_labFiles(self) -> List[UnparsedFile]:
        sql = f"""SELECT accessionNumber, xmlLabFile as file, filingDay, filingMonth, filingYear FROM {DB.SEC_REPORT_PROCESSING_TBL_NAME} WHERE xmlLabFile is not NULL and csvLabFile is NULL and labParseState is NULL"""
        return self._execute_fetchall_typed(sql, UnparsedFile)

    def update_parsed_num_file(self, updatelist: List[UpdateNumParsing]):
        update_data = [
            (x.csvNumFile, x.numParseDate, x.numParseState, x.fiscalYearEnd, x.accessionNumber)
            for x in updatelist
        ]
    ...
    def update_parsed_pre_file(self, updatelist: List[UpdatePreParsing]):
        update_data = [
            (x.csvPreFile, x.preParseDate, x.preParseState, x.accessionNumber) for x in updatelist
        ]
    ...
    def update_parsed_lab_file(self, updatelist: List[UpdateLabParsing]):
        update_data = [
            (x.csvLabFile, x.labParseDate, x.labParseState, x.accessionNumber) for x in updatelist
        ]
