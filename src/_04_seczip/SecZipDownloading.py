...
class SecZipDownloader:
    ...
    def _download_missing(self, to_download_entries: Dict[str, str]):
        for file, url in to_download_entries.items():
            result = self._downlaod_zip(url, self.zip_dir + file)
            logging.info("    {} - {}".format(file, result))
    ...
    def download(self):
        ...
        logging.info("    downloading {} to {}".format(len(zips_to_dld_dict), self.zip_dir))
        self._download_missing(zips_to_dld_dict)


if __name__ == '__main__':
    SecZipDownloader('d:/secpipeline/quarterzip').download()
