...
def _parse_xbrlfiles(edgar_sub_elem, edgar_ns, item):
    ...
    for xbrl_file in xbrl_files:
        xbrl_type = xbrl_file.attrib['{' + namespace + '}type']

        if xbrl_type == "EX-101.INS" or xbrl_type == 'EX-100.INS':
            xbrl_ins_url = xbrl_file.attrib['{' + namespace + '}url']
            xml_filing_found = True

        if xbrl_type == "EX-101.CAL" or xbrl_type == 'EX-100.CAL':
            xbrl_cal_url = xbrl_file.attrib['{' + namespace + '}url']

        if xbrl_type == "EX-101.DEF" or xbrl_type == 'EX-100.DEF':
            xbrl_def_url = xbrl_file.attrib['{' + namespace + '}url']

        if xbrl_type == "EX-101.LAB" or xbrl_type == 'EX-100.LAB':
            xbrl_lab_url = xbrl_file.attrib['{' + namespace + '}url']

        if xbrl_type == "EX-101.PRE" or xbrl_type == 'EX-100.PRE':
            xbrl_pre_url = xbrl_file.attrib['{' + namespace + '}url']
...
class SecIndexer():
    ...
    def _download_sec_feed(self, year, month):
        ...
        if not os.path.exists(feed_file):
            edgar_filings_feed = (namespace + '/monthly/' + feed_filename)
            ...
            logging.debug('Skipping download: RSS feed %s already downloaded', feed_file)
    ...
    def parse_sec_rss_feeds(self, rss_filename):
        ...
        edgar_dict = {edgar_key: [] for edgar_key in self.edgar_all_keys}
        edgar_ns = {'edgar': namespace}
        ...
        for item in items:
            ...
            for key, label in zip(self.edgar_keys, self.edgar_labels):
                ...
                if 'xbrlFiles' in edgar_sub_elem.tag:
                    ...
                    xbrl_ins_url, xbrl_cal_url, xbrl_def_url, xbrl_lab_url, xbrl_pre_url = _parse_xbrlfiles(edgar_sub_elem, edgar_ns, item)

                    # edgar_dict[key].append(xbrl_url)
                    temp_dict['xbrl_files'] = xbrl_ins_url
                    temp_dict['xbrl_cal_url'] = xbrl_cal_url
                    temp_dict['xbrl_def_url'] = xbrl_def_url
                    temp_dict['xbrl_lab_url'] = xbrl_lab_url
                    temp_dict['xbrl_pre_url'] = xbrl_pre_url
            ...
            if temp_dict['form_type'] in ['10-K','10-Q']:
    ...
    def _prep_database_table(self):
        ...
        columns = ','.join(self.edgar_all_keys)
    ...
    def _save_dicts_to_database(self, dicts):
        ...
        # db_df = db_df[db_df.form_type.isin(['10-Q','10-K'])]
