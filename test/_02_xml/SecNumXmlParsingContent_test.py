...
def read_xml() -> pd.DataFrame:
    ...
    with open(xml_file, "r", encoding="utf-8") as f:
        ...
        # df_xml['qtrs']  = df_xml.qtrs.apply(int)

        df_xml['value'] = pd.to_numeric(df_xml['value'], errors='coerce')

        df_xml['adsh'] = '0000320193-20-000096'
        df_xml.loc[df_xml.version == 'company', 'version'] = '0000320193-20-000096'

        df_xml = (df_xml[df_xml.segments.isnull()]).copy()

        df_xml.drop(['coreg', 'footnote', 'segments'], axis=1, inplace=True)
        return df_xml
